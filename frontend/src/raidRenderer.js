...
export function getRaidMapOrigin(width, height, zoomScale = 1.0) {
...
export function renderGrayscaleRaid(ctx, raidState, zoomScale = 1.0) {
  const canvas = ctx.canvas;
  const dpr = window.devicePixelRatio || 1;
  const width = canvas.clientWidth || (canvas.width / dpr);
  const height = canvas.clientHeight || (canvas.height / dpr);

  const { originX, originY, tileW, tileH } = getRaidMapOrigin(width, height, zoomScale);
